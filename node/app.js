...
app.post('/addLocation', function(req, res) { 
	...
	var maxLocId = 0; 
	// find highest loc id
	var topIdRef = firebaseRef.child('topLocId'); 
	topIdRef.once("value", function(snapshot) { 
		var topLocId = snapshot.val();
		topLocId = topLocId + 1; 
		topIdRef.set(topLocId); 
		var newLocationRef = firebaseRef.child('locations/p'+topLocId); 
		newLocationRef.set({"name": locName, "state": "New Jersey", "town": "Princeton"}); 
		res.json({"status": "success", "name": locName, "id": "p"+topLocId}); 
	    }); 
    });
